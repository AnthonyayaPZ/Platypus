...
export async function searchDictionary(query: string) {
  ...
  const [metaRow, relResult] = await Promise.all([
    pool.query(
      `SELECT uw.*, COALESCE(STRING_AGG(gw.group_id, ','), '') AS group_ids
      FROM user_words uw
...
export async function beginReviewSession(groupId: string) {
  ...
  const { rows: allRows } = await pool.query("SELECT * FROM dictionary_entries ORDER BY word");
  const allEntries: WordEntry[] = allRows.map((row: Record<string, unknown>) => parseEntry(row));
  ...
  await pool.query(
    ...
    [sessionId, DEMO_USER_ID, groupId, entries.length, tasks.length, timestamp],
  );
...
export async function answerReviewTask(sessionId: string, taskId: string, selectedAnswer: string) {
  ...
  const { rows: taskRows } = await pool.query(
    ...
    WHERE t.id = $1 AND t.session_id = $2 AND s.user_id = $3 AND s.status = 'active'`,
    [taskId, sessionId, DEMO_USER_ID],
  );
  if (!taskRows.length) throw new Error("复习题目不存在或会话已经结束");
  ...
  await batch([
    {
      text: `UPDATE review_tasks SET selected_answer = $1, is_correct = $2, answered_at = $3
        WHERE id = $4 AND answered_at IS NULL`,
      values: [selectedAnswer, isCorrect ? 1 : 0, timestamp, taskId],
    },
    {
      text: `INSERT INTO review_events
